...
public class RestrictedAreas extends Module {
    ...
    private final Setting<RenderStyle> renderStyle = sgRender
            .add(new meteordevelopment.meteorclient.settings.EnumSetting.Builder<RenderStyle>()
                    .name("render-style")
    ...
    private final Setting<String> toggleMessage = sgTeleport.add(new StringSetting.Builder()
            .name("toggle-message")
            .description(
                    "Expected message from server when running toggle command. Use (enabled/disabled) as placeholder.")
    ...
    private void onTick(TickEvent.Post event) {
        ...
        if (!autoToggleTp.get() || commandBuffer.isEmpty() || mc.player == null)
            return;
    ...
    private void onReceiveMessage(ReceiveMessageEvent event) {
        if (!autoToggleTp.get() && !autoDenyTp.get() && !autoAcceptTp.get())
            return;
                ...
                if (playerName != null && !playerName.isEmpty()) {
                    ...
                    for (var area : serverData.get().getRestrictedAreas()) {
                        ...
                        if (isInside && area.getAllowedPlayers().contains(playerName)) {
                            ...
                            messageText.append(Text.literal("automatically accepted").formatted(Formatting.GREEN,
                                    Formatting.BOLD));
    ...
    public void validateAndFixToggleState() {
        if (!autoToggleTp.get() || mc.player == null)
            return;
